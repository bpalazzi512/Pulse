...
export class PostService {
    ...
    async createPost(postDto: PostDto): Promise<Post> {
        ...
        const vote = await this.voteService.createVote({user_id, post_id: savedPost.id, vote_type: "upvote"});
        return savedPost;
    ...
    async getPostsWithVotes() : Promise<DetailedPost[]> {
        ...
        const postsToSend  = await Promise.all(posts.map(async (post : Post) => {
    ...
    async getUserPosts(userId: number) : Promise<Post[]> {
        const posts =  await this.postRepository.find({where: {user_id: userId}});
        const postsToSend  = await Promise.all(posts.map(async (post : Post) => {
            const netVotes = await this.voteService.getNetVotes(post.id);
            return {
                id: post.id,
